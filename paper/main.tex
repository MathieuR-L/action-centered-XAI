...
Crucially, enforcing causal invariance reduces out-of-distribution accuracy loss from 23.6 to 2.9 percentage points, directly demonstrating that explanation quality must be evaluated jointly with intervention robustness.
...
These systems are increasingly integrated into clinical workflows, where explanations must meet strict requirements in terms of reliability, latency, and accountability.
We examine practical deployment issues and limitations of current approaches, concluding with a roadmap for future research to advance transparent, trustworthy, and effective AI systems in healthcare.
In particular, we emphasize the gap between existing explainability methods and the needs of real-world decision support.
\textbf{Unlike prior surveys, this work argues that the ante-hoc/post-hoc taxonomy is structurally misaligned with clinical decision support.} 
While widely adopted in the literature, this distinction reflects algorithmic properties rather than end-user requirements.
Clinicians do not choose explanations based on whether they are embedded in a model or applied afterward \cite{pmlr-v106-tonekaboni19a}; they ask: ``Is this case urgent?'', ``Why this diagnosis?'', ``Why this treatment?'', ``Can I defend this decision in court?'', ``Is the model still reliable after 6 months?'' 
These questions correspond to distinct decision-making contexts that require different types of explanations.
Each question demands distinct explanation properties---speed, causality, counterfactuals, stability, temporal consistency---that current MXAI taxonomies do not foreground \cite{pmlr-v106-tonekaboni19a,gniadek2023framework} .
As a result, existing frameworks fail to capture the practical requirements of clinical use.
...
\begin{itemize}
    ...
    Med-PaLM 2 \cite{singhal2023expertlevelmedicalquestionanswering}: Achieves expert-level performance on MedQA, with up to 86.5\% accuracy, and reports strong results on MedMCQA and MMLU clinical-topic benchmarks. However, Med-PaLM 2 is primarily evaluated as a text-based medical question-answering model; its real-world clinical deployment, safety, and interpretability require further validation, while multimodal capabilities are addressed separately in Med-PaLM M.
...
\begin{tabular}{|p{2cm}|p{1.7cm}|p{4.5cm}|p{4.5cm}|}
\hline
\textbf{Clinical Action} & \textbf{Explanation Question} & \textbf{Required Properties} & \textbf{Why Current XAI Fails} \\ \hline
\textbf{Intra-operative assistance} & ``Is this case urgent?'' & Low-latency responsiveness (workflow-dependent), Uncertainty bounds, Sensitivity \textgreater 95\%
...
KernelSHAP & \textcolor{red}{\xmark} (294.16 ms) & \partialcmark ($\rho=0.80$) & \partialcmark (feature ranking) & \partialcmark (0.968 stability) & \textcolor{red}{\xmark} (no drift signal) & Slow perturbational attribution \\ \hline
LIME & \textcolor{red}{\xmark} (111.54 ms) & \partialcmark ($\rho=0.90$) & \partialcmark (local surrogate) & \partialcmark (0.951 stability) & \textcolor{red}{\xmark} (no lifecycle signal) & Hyperparameter sensitivity \\ \hline
Integrated Gradients & \partialcmark (10.60 ms) & \partialcmark ($\rho=0.80$) & \textcolor{red}{\xmark} (path attribution only) & \partialcmark (0.959 stability) & \textcolor{red}{\xmark} (no drift signal) & Baseline dependence \\ \hline
Grad-CAM & \textcolor{green}{\cmark} (3.76 ms) & \partialcmark (visual only) & \textcolor{red}{\xmark} (no CF) & \textcolor{red}{\xmark} (0.725 stability) & \textcolor{red}{\xmark} (leakage 0.031) & Metadata-induced heatmap shift \\ \hline
Attention & \textcolor{green}{\cmark} (2.75 ms) & \partialcmark (spurious token focus) & \textcolor{red}{\xmark} (no intervention) & \partialcmark (0.929 stability) & \textcolor{red}{\xmark} (no drift flag) & Spurious hospital-token focus\\ \hline
Retrieval proxy & \textcolor{green}{\cmark} (3.07 ms) & \partialcmark (purity 0.52/0.73) & \partialcmark (case support) & \textcolor{green}{\cmark} (explicit evidence) & \partialcmark (purity drop) & No generative grounding or report validation \\ \hline
Modality Ablation & \textcolor{green}{\cmark} (8.09 ms) & \textcolor{green}{\cmark} (direct intervention) & \textcolor{green}{\cmark} (counterfactual removal) & \partialcmark (0.911 stability) & \textcolor{green}{\cmark} (OOD masking reveals drift) & Intervention definition matters \\ \hline
\end{tabularx}
\par}
\textit{\cmark = suitable, \partialcmark = partially suitable, \xmark = unsuitable.}\newline

{\footnotesize
Ratings are derived from the controlled benchmark described in Section 6 (Table 6). Absolute latencies are lower bounds measured on a lightweight architecture and should be rescaled for full-resolution clinical pipelines. OOD = Out Of Distribution, CF = Counterfactual}
%Ratings are derived from the controlled benchmark: low-latency triage favored methods below 15 ms/case; diagnosis favored causal alignment and low leakage; audit favored stability or explicit evidence traces; monitoring favored direct OOD-shift observability.}
\end{table}
...
\end{tcolorbox}
From an ACE perspective, retrieval-grounded explanations represent on of the most promising current approach for satisfying the auditability requirement, as they provide explicit evidence trails linking predictions to real clinical cases.
...
\begin{itemize}
    \item \textbf{Transformers and Attention Models:} 
    ...
    \textbf{Multimodal Fusion Strategies \ref{fig:multimodal_fusion_strategies}}

    Three main fusion strategies are compared in Figure 2.
    \begin{itemize}
...
\textcolor{For instance, a transformer model integrating chest X-rays with radiology reports can attend jointly to consolidation regions in the image and the phrase 'bilateral infiltrates' in the report, providing a grounded diagnostic explanation directly actionable by the radiologist.}
...
As conlusion, methods that are fast enough for real-time use lack interventional guarantees, while those that approximate intervention are too slow or operationally sensitive for continuous deployment. 
...
Cholec80 & Video, Text & Surgical Video Analysis & 80 videos & Intraoperative \\ \hline 
...
To move several ACE assessments from literature-grounded judgment to measured evidence, we implemented a small but fully reproducible multimodal benchmark in the workspace 

(\texttt{experiments/mxai\_assessment.py}). The goal was not to reproduce a hospital-scale deployment, but to stress the core claims that underlie the ACE taxonomy: explanation latency, robustness to spurious multimodal cues, stability across retraining, and cross-modal leakage.
...
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.15}
\begin{tabularx}{\textwidth}{|L{2.8cm}|Y|Y|Y|L{5.2cm}|}
\hline
...
\end{figure}
Figure 4 shows two complementary effects of causal training. On the left, feature masking reveals that hospital-feature reliance drops from 0.517 to 0.005 accuracy points after causal training, while biomarker reliance is preserved (0.508 → 0.496), confirming that the causal objective selectively suppresses spurious dependence without degrading clinically meaningful signal. On the right, attention redistribution shows that the hospital token weight drops from 0.352 to 0.179, while image and biomarker tokens gain relative weight, a direct visualization of explanation alignment improving alongside model robustness
\section{Evaluation Metrics and Benchmarks}

Evaluation is the bottleneck of clinically viable MXAI. In healthcare, an explanation may appear faithful under standard proxy metrics while still failling to support diagnosis, misrepresenting cross-modal evidence, or breaking under distribution shift and model updates. For this reason, recent work has moved beyond purely technical XAI scores toward clinician-centered, multimodal, and regulatory evaluation criteria. This section organizes those criteria and shows how they align with the action-centered view proposed in this survey. Table \ref{tab:evaluation_frameworks} situates these requirements alongside recent clinical, multimodal, and causal evaluation frameworks for MXAI.
...
\begin{tabularx}{\textwidth}{|L{2.3cm}|L{3.8cm}|L{4.2cm}|X|}
\hline
\textbf{FDA-Oriented Requirement} & \textbf{MXAI Evaluation Question} & \textbf{Operational Metric} & \textbf{ACE-Relevant Action} \\ \hline
...
Triage, Diagnosis, Treatment \\ \hline
...
Diagnosis, Audit, Monitoring \\ \hline
...
\subsection{Research Priorities and Future Directions}
...
Our controlled benchmark showed a 41--107$\times$ runtime gap between perturbational explainers and intrinsic attention, which argues for approximation, distillation, caching, or retrieval-based evidence support rather than unconstrained online perturbation.
...
\begin{itemize}
    ...
    How to maintain stable explanations across non-i.i.d. hospital data? Our benchmark operated on a single-institution setting, leaving open the question of whether ACE properties measured locally transfer across hospital sites. In one federated lung disease setting, explanation consistency degraded by 30–45\% \cite{Durga2025flem}, a finding that warrants replication across broader MXAI contexts before drawing general conclusions.
...
Among the emerging directions surveyed, three are most urgently motivated by the ACE findings: efficient explanation pipelines to close the 41–107× latency gap between perturbational and intrinsic methods, causal attribution protocols to move beyond correlational saliency, and regulatory-grade validation frameworks to align MXAI evaluation with FDA lifecycle requirements.
